...
class Comparative:
    ...
    def get_summary(self):
        ...
        #Store Basic Information and findings
        cmd=open("cmd.txt","w")
        cmd.write("******************************************************")
        #cmd.write('\033[1m'+'\033[4m'+"General Summary: \n"+'\033[0m')
        cmd.write("\nProduct: "+ product_name)
        cmd.write("\nData file: "+ filename)
        cmd.write("\nNumber of Reviews: "+ str(num_reviews))
        cmd.write("\nNumber of Comparabale Words: "+ str(sum_compwords))
        cmd.write("\nNumber of Reviews containing comparable words: "+ str(num_comp_review))
        cmd.write("\nAverage comprable words per Review: "+ str(round(sum_compwords/num_reviews,2)))
        cmd.write("\nAverage comprable words per Comparable Review: "+ str(round(sum_compwords/num_comp_review,2)))
        cmd.write("\nPercentage of Comparable reviews from all reviews: " + str(round((num_comp_review/num_reviews)*100,2)))
        cmd.write("\nPrecision: " + str(round((precision),2)) + "\n")
        #Print the ocurrences
        #cmd.write('\033[1m'+'\033[4m'+"All Comparable Word Ocurrances: \n"+'\033[0m')
        for key in self.feature_dict:
            if self.feature_dict[key] != 0:
                cmd.write(key + ' -> ' + str(self.feature_dict[key])+"\n")
    ...
    #Execute Emotions Analysis
    def e_detection(self,x):
    ...
    def emotionAnalysis(self):
        #Read Data and put into dataframe
        df=pd.read_csv('comp.txt')
        df['Emotion'] = df[' Review'].apply(self.e_detection)
        result=df.to_html()

        #Write into emotions table into html file
        text_file = open("emotiont.html", "w")
        text_file.write(result)
        text_file.close()

        #Generate Emotions Graphic
        df['Emotion'].value_counts().plot(kind='pie',ylabel="",title="Emotions in Dataset Reviews",autopct='%.1f%%')
        plt.savefig('emotions.png')

    #Clean the files in folder by calling sh file
